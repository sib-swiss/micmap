...
/* TODO: do not count twice when the same nt is covered by both reads in a pair */
	...
	while (1) {
		...
		if (th.chr == opt->chr)
		{
			if ((th.flags_7_0 & kTLBHflagPairPosBlock) != 0)
			{
				...
				{
					// we do...
					err = readDecompressBlock(fd, &th, &td, 1);
					if (err != 0)
						goto bail;
...
main (int argc, char **argv)
{
	...
	opterr = 0;
	while ((c = getopt (argc, argv, "r:g:C:v:dct:")) != -1)
	switch (c)
	{
		case 'r':
	...
	{
		printf("usage:\n\n");
		printf("GTLcoverage [options] -C chromosome -r resultfile\n\n");
