...
#define outbyte(cod,x) putc(x, cod->stream)
#define inbyte(cod)    (int)(*cod->decstream++)
...
/* per outputbyte.                                             */
void start_encoding( rangecoder *rc, char c, int initlength, FILE * const stream )
...
int start_decoding( rangecoder *rc, const unsigned char * stream)
{   
    RNGC.decstream = stream;
    int c = M_inbyte;
    if (c==EOF)
        return EOF;
