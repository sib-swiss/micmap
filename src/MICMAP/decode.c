...
// FUNCTIONS 
//---------------------------------------------------------------
void* DecodeTags(DecoderPool_t * const restrict thpool)
{
	unsigned int packed16nt[kMAX16mersToTest];
	unsigned int mic512dump[16];
	...
	unsigned int tag16mers[8];
	unsigned int expected4mers[8];
	unsigned int expected4mers2[8];
	unsigned int candidate[8];
	...
	while(thpool->threads_keepalive) {
		...
		if (thpool->threads_keepalive){
			...
			more16mer:;
						...
						for (int m=0;m<8;m++) tag16mers[m] = packed16nt[m];

						for (int m=0; m<8; m++) expected4mers[m] = ValidDataTable->following_nt_tbl[tag16mers[m]];
						for (int m=0; m<8; m++) candidate[m] = (expected4mers[m] == 0U) ? 0xFFFFFFFF : 0U;
						unsigned int first8 = 0U;
						for (int m=31; m>=0; m--) {
							first8 <<= 1;
							unsigned char c = (((char*) candidate)[m] != 0) ? 1 : 0;
							first8 |= (unsigned int) c;
						}


						for (int m=0; m<8; m++) tag16mers[m] = packed16nt[8+m];
						for (int m=0; m<8; m++) expected4mers2[m] = ValidDataTable->following_nt_tbl[tag16mers[m]]; 
						for (int m=0; m<8; m++) candidate[m] = (expected4mers2[m] == 0U) ? 0xFFFFFFFF : 0U;
						unsigned int next8 = 0U;
						for (int m=31; m>=0; m--) {
							next8 <<= 1;
							unsigned char c = (((char*) candidate)[m] != 0) ? 1 : 0;
							next8 |= (unsigned int) c;
						}

						unsigned long j = ((unsigned long)next8 << 32) | (unsigned long) first8;
						...
						{
							for (int m=0; m<8; m++) {
								mic512dump[m]   = expected4mers[m];
								mic512dump[8+m] = expected4mers2[m];
							}
			...
			moreRev16mer:;
							...
							for (int m=0; m<8; m++) tag16mers[m] = packed16nt[m];
							for (int m=0; m<8; m++) expected4mers[m] = ValidDataTable->following_nt_tbl[tag16mers[m]];
							for (int m=0; m<8; m++) candidate[m] = (expected4mers[m] == 0) ? 0xFFFFFFFF : 0U;
							first8 = 0U;
							for (int m=31; m>=0; m--) {
								first8 <<= 1;
								unsigned char c = (((char*) candidate)[m] != 0) ? 1 : 0;
							...
							j = ((unsigned long)next8 << 32) | (unsigned long)first8;
			...
			noluckRev:;
					...
					for (unsigned int loop = 0; loop < nToBeProcessed; loop++) {
						...
						const unsigned int * restrict tbl;
						int hitA;
						int tryHits = 0;
						const unsigned short taglen = Tags[loop].AlignLen;
						for (hitA = 0; hitA < kDesiredHitCnt; hitA++)
						{
							if (rslt[loop][hitA].which16mer == 0) break; // we reached the end
						...
						while (tryHits < kDesiredHitCnt) rslt[loop][tryHits++].which16mer = 0;
					}
