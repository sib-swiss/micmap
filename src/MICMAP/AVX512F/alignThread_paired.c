...
#include "pftools_systolic_simplified.h"
#else
...
#include "../SSE4.1/alignThread_common.c"
...
#if (kTagSize*(kMaxGenomeChunkSize + kTagSize + 16 + 16)*4) > 2*HUGE_PAGE_SIZE
...
void* avx512f_pair(cpuAlignerPool_t * const restrict data)
{
	...
	_mm512_store_si512(&(stats.Counter[0]),  _mm512_setzero_si512());
	...
	int * restrict IDM_Matrix = (int *) mmap(NULL, 2*HUGE_PAGE_SIZE,
...
#undef __DECLARE_VARIABLES__
	...
	const size_t OutputBufferSize = data->AlignerBlock->OutputBlockSize;
	...
	while(1 /*data->threads_keepalive*/)
	{
			///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
					...
					if ((TreatedSoFar) < TagCount) {
						...
						for (unsigned int i = 0; i < WorkLoad; i++) {
							...
							*ptrRevTagSpace = '\0';
							...
							//Clean score vector
						_mm512_store_si512((__m512i*) &vScores[EvenOrOddOffset], __NLOW);
						_mm512_store_si512((__m512i*) &vOffsets[EvenOrOddOffset], _mm512_setzero_si512());
							...
							for (int hitnum = 0; hitnum < kDesiredHitCnt; hitnum++) {
								...
								if (mismatchCount <= 1U) {
									...
									///////////////////////////////////////////
#if MYTEST									
									if (TagsData[i].ordinal == 0)  printf("AVX512F %u anchor %i MisMatch: %u\n", i, hitnum, mismatchCount);
#endif
								...
								//-----------------------------------------------------------------------------------------------------------------------
									...
									const size_t MatrixLD     = kGAPPED_ALIGN_GENOME_LENGTH + 16 + 16;
									...
									*ptrState = '\0';
									...
									if (TagsData[i].ordinal == 0) {
										printf("AVX512F %u anchor %i OK: %i MisMatch: %u MM: %u CIG: %u SoftClip: %u Boudnary: %u %s\n",
										       i, hitnum, ret, MismatchCount, RequiredMMSize, RequiredCigSize, SoftClipMismatchCount, SoftClipBoundary, ptrState);										
									}
#endif
									if (ret == 0) {
										...
										if (MismatchCount <= kMaxMismatch) {
											...
											alntrial->score                = TO_SCORE(results[num]);
...
#ifdef DEBUG_TAG_MAPPING_PROCESS
									const int lscore = TO_SCORE(results[num]);
									printf("PFTOOLS: buf %u chunk %u trial %d/%d failed to align tag score was %i though!\n",
										((BUFFERADDR*)data)->buf, ((BUFFERADDR*)data)->chunk, hitnum, kDesiredHitCnt,lscore);
									fflush(stdout);
#endif
...
#endif
								if ((hitnum != 0)) {
									__m512i __CurrentScore  = _mm512_set1_epi32(alntrial->score);
									__m512i __CurrentOffset = _mm512_set1_epi32(KeptAnchorCount);
						...
						if (!(i&1)) {
							...
							*/
							...
							if (KeptAnchorCount > 0U) {
								...
								//--------------------------------------------------------------------------------------------------------------------------
								__m512i __Ordering, __Scores = __NLOW;
								#pragma noprefetch
								for (int EvenAnchor=0; EvenAnchor < EvenAnchorCount; EvenAnchor++) {
									...
									for (int OddAnchor=0; OddAnchor < OddAnchorCount; OddAnchor++) {
										...
										if ( EvenAlignment->chr == OddAlignment->chr \
											...
											const int score = EvenAlignment->score + OddAlignment->score;

											trials[tcnt].tag1 = EvenAlignment;
											trials[tcnt].tag2 = OddAlignment;

											__m512i __CurrentScore  = _mm512_set1_epi32(score);
											__m512i __CurrentOffset = _mm512_set1_epi32(tcnt);
								...
								if (tcnt == 0) {
									...
									for (int iAnchor=0; iAnchor<InitialKeptAnchorCount; iAnchor++) {
										...
										//----------------------------------------------------------------------------------------------------------------------
										{
											...
											const size_t MatrixLD     = (size_t) GenomeChunkSize + 16 + kTagSize + 16;
											...
											const size_t num = (ReadLength-1) & (size_t) 0xF;
#ifdef DEBUG_TAG_MAPPING_PROCESS
											printf("PFTOOLS: buf %u chunk %u trial %d/%d : score=%d found at offset %d\n",
													((BUFFERADDR*)data)->buf, ((BUFFERADDR*)data)->chunk,tcnt,kDesiredHitCnt, TO_SCORE(results[num]), Id[num]);
											printf("PFTOOLS TAG=%.*s\n"
														 "        GEN=%.*s\n", TagsData[i].TagLen, ptrTag, TagsData[i].TagLen, genome + Id[num] - TagsData[i].TagLen);
											fflush(stdout);
#endif
											int iseq = Id[num];
											const size_t EndOnMatchOrDeletion = MorD[num];
											int Align[2];
											int ret;
											unsigned int MismatchCount = 0U;
...
#if MYTEST
											if (TagsData[i].ordinal == 0) {
												printf("AVX512F %u anchor %i OK: %i MisMatch: %u MM: %u CIG: %u SoftClip: %u Boundary: %u %s\n",
				...
				// 							printf("%u\t%u\n%s\n", MismatchCount, SoftClipMismatchCount, ptrState);
											if (ret == 0 ) {
												...
												if (MismatchCount <= kMaxMismatch) {
	...
	#ifdef DEBUG_TAG_MAPPING_PROCESS
													printf("PFTOOLS: buf %u chunk %u DU UNGAP GOT IT at offset %u with %u mismatches, score was %i though!\n",
																((BUFFERADDR*)data)->buf, ((BUFFERADDR*)data)->chunk, alntrial->chr_pos,MismatchCount,alntrial->score);
	#endif
													{
														__m512i __CurrentScore  = _mm512_set1_epi32(alntrial->score);
														__m512i __CurrentOffset = _mm512_set1_epi32(KeptAnchorCount);
													...
													KeptAnchorCount++;
													{
														__m512i __CurrentScore  = _mm512_set1_epi32(PairScore);
														__m512i __CurrentOffset = _mm512_set1_epi32(tcnt);
														const __mmask16 Bigger  = _mm512_cmpge_epi32_mask(__CurrentScore, __Scores);
														__m512i __newScores     = _mm512_mask_mov_epi32(__Scores, _mm512_knot(Bigger), __CurrentScore);
														__m512i __newOffsets    = _mm512_mask_mov_epi32(__Ordering, _mm512_knot(Bigger), __CurrentOffset);
								...
								if (tcnt > 0) {
									int PairOrdering[16] __attribute__((aligned(64)));
									int PairScores[16] __attribute__((aligned(64)));
									_mm512_store_epi32(PairOrdering, __Ordering);
									_mm512_store_epi32(PairScores, __Scores);
...
//  						printf("Appolo Thread %u job results fully stored, requesting new job for buffer %u\n", MyID, CurrentInputIndex); fflush(stdout);
						if(recvBufferFromHost(AlignPool, &Jobs[CurrentInputIndex]) != 0) {
							...
							/* Place the space back into its queuei, setting the count to 0 to avoid flushing later */
...
// 	 					printf("Appolo Thread %u working on job %i\n", MyID, CurrentInputIndex);fflush(stdout);
	...
	_mm512_store_si512(&(data->stats[MyID]), *((__m512i *) &(stats.Counter[0])));
