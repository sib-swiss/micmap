...
{
		printf("usage:\n\n");
		printf("GTLdiffCaller3 [options] -r glob GTL reference files -t glob GTL other files\n\n");
				...
				if (task->TotalJobs == 0 ) {
					...
					if ((td->header.flags_7_0 & kTLBHflagPairedReads)) {
						...
						for (int i=0; i<td->cnt; i++) {
							...
							assert(Block->provenance < 2);
							ND[0]->Provenance = Block->provenance;
							ND[1]->Provenance = Block->provenance;
							// Filter duplicates...
							int keep = 1;
...
int main (int argc, char **argv)
{
	...
	const char * restrict GTLReferenceInputFilePattern = NULL;
	GTLList_t * restrict GTLReferenceInputFileList = NULL;
	const char * restrict GTLOtherInputFilePattern = NULL;
	GTLList_t * restrict GTLOtherInputFileList = NULL;
	threadpool_t * restrict thpool = NULL;
	cpu_set_t * affinities = NULL;

	glob_t GTLReferenceGlob, GTLOtherGlob;
	...
	// Process arguments
	opterr = 0;
	while ((c = getopt (argc, argv, "g:cC:v:f:m:T:w:s:V:ahzWr:t:")) != -1)
	switch (c)
	{
		case 'r':
			GTLReferenceInputFilePattern = optarg;
			break;

		case 't':
			GTLOtherInputFilePattern = optarg;
			break;
	...
	if ( !Genome.FileName) {
		fputs("You need to provide a genome!\n", stderr);
		exit(1);
	}
	...
	if (GTLOtherInputFilePattern == NULL || GTLReferenceInputFilePattern == NULL) {
		usage();
	}

	/* Reference input files */
	if(glob(GTLReferenceInputFilePattern, GLOB_NOCHECK | GLOB_TILDE, 0, &GTLReferenceGlob)) {
		fprintf(stderr, "Error in globbing reference file pattern %s\n", GTLReferenceInputFilePattern);
		exit(1);
	}
	if (verbose) fprintf(stderr,"%lu reference GTL files provided\n", GTLReferenceGlob.gl_pathc);

	/* Other intpu file names */
	if(glob(GTLOtherInputFilePattern, GLOB_NOCHECK | GLOB_TILDE, 0, &GTLOtherGlob)) {
		fprintf(stderr, "Error in globbing other file pattern %s\n", GTLOtherInputFilePattern);
		exit(1);
	}
	if (verbose) fprintf(stderr,"%lu other GTL files provided\n", GTLOtherGlob.gl_pathc);

	if (verbose) {
		fprintf(stderr, "Will use %u threads for fetching data, %u for analyzing\n"
		"Filter set to character '%c'\n", nFetchThreads, nAnalizeThreads, options.filterQualValue);
	}
	...
	GTLReferenceInputFileList = createGTLList((const char * const restrict* const) GTLReferenceGlob.gl_pathv, GTLReferenceGlob.gl_pathc);
	if (GTLReferenceInputFileList == NULL) {
		fputs("Error while preparing reference GTL input files\n", stderr);
		goto bail;
	}

	if (indexGTLList(GTLReferenceInputFileList, Range, false, 0)) {
		fputs("Error indexing reference GTL list\n", stderr);
		goto bail;
	}
	fprintf(stderr, "Found %lu reference blocks\n", GTLReferenceInputFileList->nBlocks);
	if ( GTLReferenceInputFileList->PositionType != Range ) {
		fputs("Getting reference real Block position extrema\n", stderr);
		getRealBlockMinMaxPosition( GTLReferenceInputFileList, nFetchThreads, 1 /*DoSort*/);
		fprintf(stderr, "Writing those reference indices to file...\n");
		if(writeGTLFileIndex(GTLReferenceInputFileList, NULL) != 0) {
			fputs("Error in writing reference indices...\n", stderr);
		}
	}
	else {
		sortGTLlist(GTLReferenceInputFileList);
	}

	GTLOtherInputFileList = createGTLList((const char * const restrict* const) GTLOtherGlob.gl_pathv, GTLOtherGlob.gl_pathc);
	if (GTLOtherInputFileList == NULL) {
		fputs("Error while preparing other GTL input files\n", stderr);
		goto bail;
	}

	if (indexGTLList(GTLOtherInputFileList, Range, false, 1)) {
		fputs("Error indexing other GTL list\n", stderr);
		goto bail;
	}
	fprintf(stderr, "Found %lu other blocks\n", GTLOtherInputFileList->nBlocks);
	if ( GTLOtherInputFileList->PositionType != Range ) {
		fputs("Getting other real Block position extrema\n", stderr);
		getRealBlockMinMaxPosition( GTLOtherInputFileList, nFetchThreads, 1 /*DoSort*/);
		fprintf(stderr, "Writing those other indices to file...\n");
		if(writeGTLFileIndex(GTLOtherInputFileList, NULL) != 0) {
			fputs("Error in writing other indices...\n", stderr);
		}
	}
	else {
		sortGTLlist(GTLReferenceInputFileList);
...
// 	thpool->common = (volatile void * volatile) &dispatcherWindow[0]; // Check this out, I do not think it is necessary anymore
	...
	IntervalResultsPool = createIntervalResultsPool(DiffAnalyzeTags, affinities ? &affinities[nFetchThreads] : NULL, nAnalizeThreads, 0);
	...
	{
		const size_t nBlocks = GTLReferenceInputFileList->nBlocks;
		for (size_t iBlock=0; iBlock<nBlocks; iBlock++) {
			if (GTLReferenceInputFileList->Blocks[iBlock].thd.minPos < InputChrRangeBegin)
				InputChrRangeBegin = GTLReferenceInputFileList->Blocks[iBlock].thd.minPos;
			if (GTLReferenceInputFileList->Blocks[iBlock].thd.maxPos > InputChrRangeEnd)
				InputChrRangeEnd = GTLReferenceInputFileList->Blocks[iBlock].thd.maxPos;
		}
	...
	printf("##phasing=none\n"
	       "##source=GTL Diff Caller 3 - " GTL_VERSION_FULL_STRING "\n"
...
// 		 			printf("Dispatching window %u-%u\n", dispatcherWindow[0], dispatcherWindow[1]);
					if (PreviousWindow[0] != dispatcherWindow[0] && PreviousWindow[1] != dispatcherWindow[1]) {
						...
						dispatchCount  = dispatchGTLBlockRange(GTLReferenceInputFileList, thpool, options.chr,
						                                       dispatcherWindow[0], dispatcherWindow[1], -1, false, work);
						dispatchCount += dispatchGTLBlockRange(GTLOtherInputFileList, thpool, options.chr,
																									 dispatcherWindow[0], dispatcherWindow[1], -1, false, work);
						if (dispatchCount) {
							/* Get a ending job for the task */
							GTLBlockDispatchRange_t * restrict EndJob;
							do {
								bsem_wait(thpool->donequeue_p.has_items);
								pthread_mutex_lock(&(thpool->donequeue_p.rwmutex));
								EndJob = (GTLBlockDispatchRange_t *) jobqueue_pull(&(thpool->donequeue_p));
								pthread_mutex_unlock(&(thpool->donequeue_p.rwmutex));
							} while (EndJob == NULL);
							EndJob->TotalJobs = dispatchCount;
							EndJob->params = work;
							EndJob->GenomeStart = dispatcherWindow[0];
							EndJob->GenomeEnd = dispatcherWindow[1];
							EndJob->Block = NULL;

							pthread_mutex_lock(&(thpool->jobqueue_p.rwmutex));
							jobqueue_push(&(thpool->jobqueue_p), (job_t*) EndJob);
							pthread_mutex_unlock(&(thpool->jobqueue_p.rwmutex));
						}
						else {
						...
						if ((++GarbageCollect & 0xFFF) == 0) {
							...
							for (size_t iBlock=0; iBlock<GTLReferenceInputFileList->nBlocks; iBlock++) {
								if (GTLReferenceInputFileList->Blocks[iBlock].thd.maxPos < dispatcherWindow[0]) {
									if (GTLReferenceInputFileList->Blocks[iBlock].whatever != NULL && GTLReferenceInputFileList->Blocks[iBlock].usingIt == 0) {
										pthread_mutex_lock(&(GTLReferenceInputFileList->Blocks[iBlock].decompressing));
										freeBlock(GTLReferenceInputFileList->Blocks[iBlock].whatever);
										GTLReferenceInputFileList->Blocks[iBlock].whatever = NULL;
										pthread_mutex_unlock(&(GTLReferenceInputFileList->Blocks[iBlock].decompressing));
									}
								}
							}
							GarbageCollect = 0U;
		...
		// No VCF file
			...
			unsigned int ChrRangeEnd = 0U;
			{
				const size_t nBlocks = GTLReferenceInputFileList->nBlocks;
				for (size_t iBlock=0; iBlock<nBlocks; iBlock++) {
					if (GTLReferenceInputFileList->Blocks[iBlock].thd.minPos < ChrRangeBegin)
						ChrRangeBegin = GTLReferenceInputFileList->Blocks[iBlock].thd.minPos;
					if (GTLReferenceInputFileList->Blocks[iBlock].thd.maxPos > ChrRangeEnd)
						ChrRangeEnd = GTLReferenceInputFileList->Blocks[iBlock].thd.maxPos;
				}
			...
			do {
				...
				DispatchCount  = dispatchGTLBlockRange(GTLReferenceInputFileList, thpool, options.chr,
				                                       work->GenomeStart, work->GenomeEnd, -1, false, work);
				DispatchCount += dispatchGTLBlockRange(GTLOtherInputFileList, thpool, options.chr,
																						   work->GenomeStart, work->GenomeEnd, -1, false, work);
				if (DispatchCount) {
					/* Get a ending job for the task */
					...
					EndJob->TotalJobs = DispatchCount;
					EndJob->params = work;
					EndJob->GenomeStart = work->GenomeStart;
					EndJob->GenomeEnd = work->GenomeEnd;
					EndJob->Block = NULL;
				...
				if ((++GarbageCollect & 0xFFF) == 0) {
					/* Free potential blocks */
					for (size_t iBlock=0; iBlock<GTLReferenceInputFileList->nBlocks; iBlock++) {
						if (GTLReferenceInputFileList->Blocks[iBlock].thd.maxPos < work->GenomeStart) {
							if (GTLReferenceInputFileList->Blocks[iBlock].whatever != NULL && GTLReferenceInputFileList->Blocks[iBlock].usingIt == 0) {
								pthread_mutex_lock(&(GTLReferenceInputFileList->Blocks[iBlock].decompressing));
								freeBlock(GTLReferenceInputFileList->Blocks[iBlock].whatever);
								GTLReferenceInputFileList->Blocks[iBlock].whatever = NULL;
		...
		for (size_t iLine=0UL; iLine < nBedFileLines; iLine++) {
			...
			do {
				...
				if ((++GarbageCollect & 0xFFF) == 0) {
					...
					for (size_t iBlock=0; iBlock<GTLReferenceInputFileList->nBlocks; iBlock++) {
						if (GTLReferenceInputFileList->Blocks[iBlock].thd.maxPos < work->GenomeStart) {
							if (GTLReferenceInputFileList->Blocks[iBlock].whatever != NULL
								&& GTLReferenceInputFileList->Blocks[iBlock].usingIt == 0) {
								pthread_mutex_lock(&(GTLReferenceInputFileList->Blocks[iBlock].decompressing));
								freeBlock(GTLReferenceInputFileList->Blocks[iBlock].whatever);
								free(GTLReferenceInputFileList->Blocks[iBlock].whatever);
								GTLReferenceInputFileList->Blocks[iBlock].whatever = NULL;
								pthread_mutex_unlock(&(GTLReferenceInputFileList->Blocks[iBlock].decompressing));
							}
						}
...
bail:;
	fflush(stdout);
	if (GTLReferenceInputFileList) destroyGTLList(GTLReferenceInputFileList);

	FreeGenome(&Genome);

	globfree(&GTLReferenceGlob);
	globfree(&GTLOtherGlob);
