...
//       lScore = _mm_extract_epi32(__max1, DUMMY);
   ...
   */
  {
    ...
    __m128i __Scores = _mm_set1_epi32((unsigned int) NLOW<<SCORE_SHIFT);
#endif
		/*
     * LOOP THROUGH THE REST OF THE PROFILE
     */
    for (size_t iprf=1; iprf<=prfLength; ++iprf) {
      ...
      KOPM = IOP_R[iprf].Element.Match;
#ifdef ONLY_NECESSARY
      int tM = ((KM + (int) Transitions[iprf].Element[_MD]) << SCORE_SHIFT ) | PRIORITY_MATCH;
      int tX = ((     (int) Transitions[iprf].Element[_XD]) << SCORE_SHIFT ) | PRIORITY_EXTRA;
      int tI = ((KI + (int) Transitions[iprf].Element[_ID]) << SCORE_SHIFT ) | PRIORITY_INSERTION;
      int tD = ((KD + (int) Transitions[iprf].Element[_DD]) << SCORE_SHIFT ) | PRIORITY_DELETION;
      int tIOPD1 = MAX(tM, tX);
      int tIOPD2 = MAX(tI, tD);
      KOPD     = MAX( tIOPD1, tIOPD2);
      __Scores = _mm_insert_epi32(__Scores, KOPD, DELETION);

      tM = ((KM + (int) LastSequenceProtein[iprf].From[MATCH])     << SCORE_SHIFT ) | PRIORITY_MATCH;
      tI = ((KI + (int) LastSequenceProtein[iprf].From[INSERTION]) << SCORE_SHIFT ) | PRIORITY_INSERTION;
      tD = ((KD + (int) LastSequenceProtein[iprf].From[DELETION])  << SCORE_SHIFT ) | PRIORITY_DELETION;
      ...
      KOPD = (KOPD < 0) ? (KOPD >> SCORE_SHIFT) | 0xC0000000 : (KOPD >> SCORE_SHIFT);
#else
      // Transform KM into a vector
      __m128i __KM = _mm_set1_epi32(KM);
      // Load Transitions and Convert signed WORD into signed DWORD
	...
	do {
		...
		if (!AlreadyUsed && (lAlignment.Score >= prf->CutOff) ) {
			...
			if (Alignment_SeqLength > StateLength) {
 			...
 			GetStateSequence(matrix, StateSequence, lAlignment.Matrix.array, ld, Alignment_SeqLength);
...
const Compute_t Standard_sse41 = {
